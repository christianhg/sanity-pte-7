...
export function usePortableTextMemberItemsFromProps(
  ...
  return useMemo(() => {
    ...
    /**
     * Register a block-level array item (text block or block-object) and recurse
     * into any nested array-of-objects fields it contains. This handles both the
     * classic depth-2 cases (block.children, block.markDefs) and Container API v2
     * cases (e.g. list.items[].content[].image), letting member items exist for
     * objects at any nesting depth so click-to-open-dialog works in containers.
     */
    function collectFromBlockItem(
      item: ArrayOfObjectsItemMember,
      acc: typeof result,
    ): void {
      const isObjectBlock = !isBlockType(item.item.schemaType)
      if (isObjectBlock) {
        acc.push({kind: 'objectBlock', member: item, node: item.item})
        // Recurse into any nested array-of-objects fields of this block-object.
        // Container API v2: a block-object can declare an `arrayField` (e.g.
        // `list.items`, `list-item.content`) that contains more PT nodes.
        for (const field of item.item.members) {
          ...
          for (const child of field.field.members) {
            if (child.kind === 'item') {
              collectFromBlockItem(child, acc)
            }
          }
        }
        return
      ...
      // Text block branch (unchanged from the depth-2 implementation).
      ...
      if (
        ...
        acc.push({kind: 'textBlock', member: item, node: item.item})
      }
      // Inline objects
      ...
      if (
        ...
        for (const child of childrenField.field.members) {
          if (child.kind === 'item' && child.item.schemaType.name !== 'span') {
            acc.push({kind: 'inlineObject', member: child, node: child.item})
          }
        }
      }
      // Markdefs
      ...
      if (markDefArrayMember) {
        for (const child of markDefArrayMember.field.members) {
          if (child.kind === 'item' && child.item.schemaType.jsonType === 'object') {
            acc.push({kind: 'annotation', member: child, node: child.item})
          }
        }
      }
    }
    ...
    for (const member of members) {
      if (member.kind === 'item') {
        collectFromBlockItem(member, result)
      }
    }
